...
int vc_gray_to_binary(IVC *src, IVC *dst, int threshold)
{
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			...
			// Para conseguir extrair apenas o cerebro adicionamos tambem para um threshold maior que 175
...
// Função para extrair apenas o cerebro a partir da imagem obtida na ultima dilatacao
int vc_brains_out(IVC *original, IVC *src, IVC *dst)
{
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			...
			// Onde a imagem é preta não faz nada
			if (datasrc[pos] == 0)
				continue;
			// Onde a imagem é branca, a imagem de destino fica com as cores da imagem original
			else
			{
